...
class flatten(object):
    ...
    def backward(self, dprev):
        ...
        #############################################################################
        #pass
        dfeat = dprev.reshape(self.prev_shape)
        #############################################################################
        #                             END OF YOUR CODE                              #
        #############################################################################
        self.meta = None
...
class fc(object):
    ...
    def forward(self, feat):
        ...
        if feat.shape[0] is None:
            res = feat.reshape(1, self.input_dim)
        else:
            res = feat.reshape((feat.shape[0]), self.input_dim)
        
        output = res.dot(self.params[self.w_name]) + self.params[self.b_name].T
        #############################################################################
        #                             END OF YOUR CODE                              #
        #############################################################################
        self.meta = feat
...
class leaky_relu(object):
    ...
    def forward(self, feat):
        ...
        mask = np.where(feat<0,self.negative_slope,1)
...
class dropout(object):
    ...
    def forward(self, feat, is_training=True, seed=None):
        ...
        if is_training and self.keep_prob > 0:
           kept = (self.rng.uniform(0.0, 1.0, feat.shape) >= (1- self.keep_prob)).astype(float) * (1.0 / self.keep_prob)
           output = feat * kept
        else:
